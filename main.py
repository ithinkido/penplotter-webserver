...
from flask import Flask, Response, render_template, request, redirect, url_for, abort, send_from_directory, jsonify, send_file
...
from  preview import HPGL_preview
...
def preview_file():
    if request.method == "POST":
        ...
        if os.path.exists(input_file):
            return  HPGL_preview(input_file)
        else:
            socketio.emit('error', {'data': 'The file does not exist'})
            return 'The file does not exist'
